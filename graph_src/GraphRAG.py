	...
	def search(self, question, options, max_num_pairs=10, paths_for_pair=2, retrieve_definitions=True):
		...
		try:
			...
			for entity in entities:
				result = self.entity_linker.link(entity)
				
				if isinstance(result, list) and all(isinstance(r, list) for r in result):
					linked_entities.extend(result) # If it has duplicates add them both, es:
					# [[127478, 'exposure', 'Phenol'], [17377, 'drug', 'Phenol']]
				else:
					linked_entities.append(result)

			linked_entities = list({tuple(e) for e in linked_entities})
