...
if __name__ == "__main__":
    fnames = sorted([fname.name for fname in (PROJECT_DIR / "corpus" / "statpearls" / "statpearls_NBK430685").iterdir() if fname.name.endswith("nxml")])
    if not (PROJECT_DIR / "corpus" / "statpearls" / "chunk").exists():
        (PROJECT_DIR / "corpus" / "statpearls" / "chunk").mkdir(parents=True, exist_ok=True)
    for fname in tqdm.tqdm(fnames):
        fpath = PROJECT_DIR / "corpus" / "statpearls" / "statpearls_NBK430685" / fname
        saved_text = extract(fpath)
        if len(saved_text) > 0:
            with open(PROJECT_DIR / "corpus" / "statpearls" / "chunk" / fname.replace(".nxml", ".jsonl"), 'w') as f:
                f.write('\n'.join(saved_text))