...
def embed(chunk_dir, index_dir, model_name, **kwarg):
	...
	if "contriever" in model_name:
		...
		model = CustomizeSentenceTransformer(model_name, device=device)
...
class Retriever: 
	def __init__(self, retriever_name="ncbi/MedCPT-Query-Encoder", corpus_name="textbooks", db_dir=PROJECT_DIR/"corpus", HNSW=False, **kwarg):
		...
		if "bm25" in self.retriever_name.lower():
			...
			if "contriever" in self.retriever_name.lower():
				self.embedding_function = SentenceTransformer(self.retriever_name, device=self.device)
			else:
				self.embedding_function = CustomizeSentenceTransformer(self.retriever_name, device=self.device)
...
class RetrievalSystem:
	def __init__(self, retriever_name="MedCPT", corpus_name="Textbooks", db_dir=PROJECT_DIR/"corpus", HNSW=False, cache=False):
		self.retriever_name = retriever_name
		self.corpus_name = corpus_name
...
class DocExtracter:
	def __init__(self, db_dir=PROJECT_DIR/"corpus", cache=False, corpus_name="MedCorp"):
		self.db_dir = db_dir
		self.cache = cache	
